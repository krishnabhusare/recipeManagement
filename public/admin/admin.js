...
function banUsers() {
    ...
    axios.get('http://13.234.231.9:3000/follow/get-usersToFollow', { headers: { Authorization: token } })
        .then(result => {
            document.getElementById('banneduser').innerHTML = "";
...
function showUsersOnScreen(obj) {
    document.getElementById('banneduser').innerHTML += `<li id="${obj.id}">${obj.name}<button onclick="banned(${obj.id})">ban</button></li>`
}

function banned(userid) {
    axios.delete(`http://13.234.231.9:3000/admin/delete-user/${userid}`, { headers: { Authorization: token } })
        .then(result => {
...
function showDetailedRecipeOnScreen(obj) {
    ...
    document.getElementById('myrecipe').innerHTML += `<h3 style= "color:red">${obj.recipename}</h3><p>${obj.description}</p><br><li>cooking time:${obj.cookingtime}</li> <li>Serves:${obj.serves}</li>
